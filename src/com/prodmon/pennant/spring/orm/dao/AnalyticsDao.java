...
import com.prodmon.pennant.spring.orm.model.ModuleSummary;
import com.prodmon.pennant.spring.orm.model.Past30CompletionSummary;
import com.prodmon.pennant.spring.orm.model.ProjectSummary;
import com.prodmon.pennant.spring.orm.model.SubtaskSummary;
import com.prodmon.pennant.spring.orm.model.TaskSummary;
...
//				+ "GROUP BY p.proj_id, p.proj_name";
		...
		String sql = "SELECT p.proj_id, p.proj_name, ABS(SUM(EXTRACT(EPOCH FROM (t.task_cmp_datetime - t.task_cdatetime))) / 3600) AS total_working_hours "
		           + "FROM s4_Projects p " + "JOIN s4_ProjectTasks pt ON p.proj_id = pt.proj_id "
		           + "JOIN s4_Tasks t ON pt.task_id = t.task_id " + "WHERE pt.task_user_id = ? "
		           + "GROUP BY p.proj_id, p.proj_name";
...
//				+ "GROUP BY m.modl_id, m.modl_name";
		...
		String sql = "SELECT m.modl_id, m.modl_name, ABS(SUM(EXTRACT(EPOCH FROM (t.task_cmp_datetime - t.task_cdatetime))) / 3600) AS total_working_hours "
		           + "FROM s4_ProjectModules m " + "JOIN s4_ProjectTasks pt ON m.modl_id = pt.task_modl_id "
		           + "JOIN s4_Tasks t ON pt.task_id = t.task_id " + "WHERE pt.task_user_id = ? "
		           + "GROUP BY m.modl_id, m.modl_name";
...
//				+ "WHERE pt.task_user_id = ? " + "GROUP BY t.task_id, t.task_name";
		String sql = "SELECT t.task_id, t.task_name, ABS(SUM(EXTRACT(EPOCH FROM (t.task_cmp_datetime - t.task_cdatetime))) / 3600) AS total_working_hours "
		           + "FROM s4_Tasks t " + "JOIN s4_ProjectTasks pt ON t.task_id = pt.task_id "
		           + "WHERE pt.task_user_id = ? " + "GROUP BY t.task_id, t.task_name";
...
//				+ "GROUP BY st.sbts_id";
		...
		String sql = "SELECT st.sbts_id, ABS(SUM(EXTRACT(EPOCH FROM (t.task_cmp_datetime - t.task_cdatetime))) / 3600) AS total_working_hours "
		           + "FROM s4_SubTasks st " + "JOIN s4_Tasks t ON st.task_id = t.task_id "
		           + "JOIN s4_ProjectTasks pt ON t.task_id = pt.task_id " + "WHERE pt.task_user_id = ? "
		           + "GROUP BY st.sbts_id";
...
//				+ "GROUP BY generated_dates.completed_date " + "ORDER BY generated_dates.completed_date DESC";
		...
		String sql = "SELECT generated_dates.completed_date::date, " 
		           + "COUNT(DISTINCT t.task_id) AS tasks_completed, "
		           + "COUNT(DISTINCT st.sbts_id) AS subtasks_completed "
		           + "FROM (SELECT (CURRENT_DATE - SEQUENCE.DAY * INTERVAL '1 day') AS completed_date "
		           + "      FROM generate_series(0, 29) AS SEQUENCE(DAY)) generated_dates "
		           + "LEFT JOIN s4_Tasks t ON DATE_TRUNC('day', t.task_cmp_datetime) = generated_dates.completed_date "
		           + "LEFT JOIN s4_SubTasks st ON st.task_id = t.task_id AND DATE_TRUNC('day', t.task_cmp_datetime) = generated_dates.completed_date "
		           + "GROUP BY generated_dates.completed_date "
		           + "ORDER BY generated_dates.completed_date DESC";
